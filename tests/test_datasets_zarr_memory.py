...
def generate_group(height=2048, width=2048, chunk_height=512, chunk_width=512,
    ...
    data_grp.create_dataset("0", data=img, chunks=(3, chunk_height,
                                                   chunk_width))
    data_grp.create_dataset("1", data=img[:, ::2, ::2],
                            chunks=(3, chunk_height // 2, chunk_width // 2))
    data_grp.create_dataset("2", data=img[:, ::4, ::4],
                            chunks=(3, chunk_height // 4, chunk_width // 4))
...
@pytest.fixture
def generate_groups():
    grp_1 = generate_group(height=2048, width=2048, chunk_height=512,
                           chunk_width=512,
                           coeff_1=2,
                           coeff_2=2,
                           label=0,
                           mask_threshold=195)
    grp_2 = generate_group(height=1024, width=2048, chunk_height=512,
                           chunk_width=512,
                           coeff_1=2,
                           coeff_2=3,
                           label=1,
                           mask_threshold=195)
    grp_3 = generate_group(height=2048, width=1024, chunk_height=512,
                           chunk_width=512,
                           coeff_1=3,
                           coeff_2=2,
                           label=2,
...
def test_zarrdataset(generate_groups, generate_input_transform):
    ...
    for s in range(len(generate_groups)):        
        data_shape = generate_groups[s][data_group].shape
...
def test_fully_labeled_zarrdataset(generate_groups, generate_input_transform,
    ...
    for s in range(len(generate_groups)):
        data_shape = generate_groups[s][data_group].shape
        labels_shape = generate_groups[s][labels_data_group].shape
...
def test_masked_zarrdataset(generate_groups, generate_input_transform):
    ...
    data_shape = generate_groups[0][data_group].shape
