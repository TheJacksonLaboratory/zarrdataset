...
def generate_groups():
    ...
    return grps, mask_grps, labels_grps
...
def test_zarrdataset(generate_groups, generate_input_transform):
    ...
    for s in range(len(image_groups)):        
        with Image.open(image_groups[s]) as im:
...
def test_fully_labeled_zarrdataset(generate_groups, generate_input_transform,
    ...
    for s in range(len(image_groups)):
        ...
        with Image.open(labels_groups[s]) as im:
            labels_shape = (im.height, im.width)
...
def test_masked_zarrdataset(generate_groups, generate_input_transform):
    ...
    with Image.open(image_groups[0]) as im:
        data_shape = [im.height, im.width, len(im.getbands())]
...
def test_generated_mask_labeled_zarrdataset(generate_groups,
    ...
    with Image.open(labels_groups[0]) as im:
        labes_shape = [im.height, im.width, 3]
